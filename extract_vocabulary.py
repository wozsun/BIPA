...
def add_vocabulary_to_file(file_path, vocabulary):
    ...
    功能说明:
        - 在frontmatter后插入词汇表，如果没有frontmatter则在文件开头
        - 如果文件已存在词汇表，则替换为新版本
    ...
    try:
        ...
        if original_content.startswith('---\n'):
            ...
            if end_pos != -1:
                frontmatter = original_content[:end_pos + 5]  # 包含结束的---\n
                content_start = end_pos + 5

                # 确保frontmatter后有空行
                if not frontmatter.endswith('\n'):
                    frontmatter += '\n'
            else:
                # 如果没有找到结束标记，当作没有frontmatter处理
                frontmatter = ""
                content_start = 0

        # 获取除frontmatter外的内容
        main_content = original_content[content_start:].lstrip('\n')  # 移除开头的多余空行
        ...
        # 检查主内容中是否已经存在词汇表，如果存在则替换
        if "**词汇表**" in main_content:
            # 找到词汇表的结束位置（下一个---或文件末尾）
            vocab_start = main_content.find("**词汇表**")
            vocab_end = main_content.find("\n---\n", vocab_start)
            if vocab_end == -1:
                # 如果找不到结束标记，查找下一个#标题
                vocab_end = main_content.find("\n# ", vocab_start)
                if vocab_end == -1:
                    vocab_end = len(main_content)
            else:
                vocab_end += 5  # 包含\n---\n

            # 替换现有词汇表
            new_main_content = vocab_content + main_content[vocab_end:].lstrip('\n')
        else:
            # 添加新词汇表到主内容开头
            new_main_content = vocab_content + main_content

        # 组合最终内容：frontmatter + 空行 + 词汇表 + 其他内容
        if frontmatter:
            new_content = frontmatter + '\n' + new_main_content
        else:
            new_content = new_main_content
...
def extract_vocabulary_from_file(file_path):
    ...
    # 如果文件以frontmatter开头，跳过frontmatter部分
    if content.startswith('---\n'):
        # 查找frontmatter的结束位置
        end_pos = content.find('\n---\n', 4)
        if end_pos != -1:
            content = content[end_pos + 5:]  # 跳过frontmatter
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    ...
    # 查找所有BIPA3下的词汇文件
    base_path = "/Users/wozsun/Library/Mobile Documents/iCloud~md~obsidian/Documents/BIPA/BIPA3"
...
"""
    ...
    # 保存到BIPA3目录
    output_path = "/Users/wozsun/Library/Mobile Documents/iCloud~md~obsidian/Documents/BIPA/BIPA3/Kosakata.md"
